function [ t,Y,N_nodes,b_thr] = mcintyre(dur,data,stim_dur,fun_type,custom_fun,fiberD,frq,end_on_ap,I_intra,N_intra)
    b_thr = 0;
    isi = 1/frq*1000; %ms
    displacement=0;
    pol=1;
    ...
    fiberD_l = [0 2 5.7 7.3 8.7 10.0 11.5 12.8 14.0 15.0 16.0];   
    nodeD_l=[0 1.4 1.9 2.4 2.8 3.3 3.7 4.2 4.7 5.0 5.5];              
    axonD_l=[0 1.6 3.4 4.6 5.8 6.9 8.1 9.2 10.4 11.5 12.7];
    deltax_l=[0 373.2 500 750 1000 1150 1250 1350 1400 1450 1500];     
    flutlength_l=[0 10 35 38 40 46 50 54 56 58 60];   
    nl_l=[0 30 80 100 110 120 130 135 140 145 150];    

    
    axonD = interp1(fiberD_l,axonD_l,fiberD,'linear','extrap');
    nodeD = interp1(fiberD_l,nodeD_l,fiberD,'linear','extrap');
    mysaD = nodeD;
    flutD = axonD;
    deltax = interp1(fiberD_l,deltax_l,fiberD,'linear','extrap');
    flutlength = interp1(fiberD_l,flutlength_l,fiberD,'linear','extrap');
    nl = interp1(fiberD_l,nl_l,fiberD,'linear','extrap');
    ...
    [V_stim,N_nodes,x_af,af,Xlr_stim] = interpolate(data);
    Xlr=Xlr_stim;
    ...
    function [Ve,N,x_af,af,Vlr] = interpolate(data)
        ...
        x_n=x_n+displacement*1e-4*deltax/10;
        x_m=x_m+displacement*1e-4*deltax/10;
        x_f=x_f+displacement*1e-4*deltax/10;
        x_i=x_i+displacement*1e-4*deltax/10;
        xlr=xlr+displacement*1e-4*deltax/10;
        
        %xd=0:0.001:3;
        %Vxd =interp1(s,Ve_pulse,xd,'linear','extrap');
